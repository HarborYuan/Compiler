...
{
fprintf(stderr,
"Can't get memory to duplicate set\n");
exit(1);
}
...
{
new->
map = (_SETTYPE *) malloc(set->nwords * sizeof(_SETTYPE));
if ( !new->map )
{
fprintf(stderr,
"Can't get memory to duplicate set bit map\n");
exit(1);
}
...
{
fprintf(stderr,
"Can't get memory to expand set\n");
exit( 1 );
}
...
map = new;
set->
nwords = (unsigned char) need;
set->
nbits = need * _BITS_IN_WORD;
}
